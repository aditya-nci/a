...
function initializeProjectCards() {
    ...
    projectCards.forEach(card => {
        ...
        // Remove modal click handler - let the original links work
    });
}
...
function openProjectModal(projectId) {
    ...
    // Create simple iframe immediately (no skeleton loading for now)
    modalBody.innerHTML = `
        <iframe 
            src="${projectUrl}" 
            style="width: 100%; height: 100%; border: none; border-radius: 0;"
            onload="console.log('Iframe loaded for ${projectId}')"
            onerror="console.error('Iframe error for ${projectId}')"
        ></iframe>
    `;
...
function closeProjectModal() {
    ...
    // Create iframe to load the actual project detail page
    ...
    // Handle iframe load to sync theme and show content
    iframe.onload = function() {
        // Sync theme with main page
        syncThemeWithIframe(iframe);
        
        // Hide skeleton and show iframe with fade-in effect
        setTimeout(() => {
            const loadingElement = modalBody.querySelector('.modal-loading');
            if (loadingElement) {
...
// Function to sync theme between main page and iframe
function syncThemeWithIframe(iframe) {
    try {
        const isDark = document.body.classList.contains('dark');
        const activeStyleLink = document.querySelector('link[class="alternate-style"]:not([disabled])');
        const activeStyle = activeStyleLink ? activeStyleLink.getAttribute('title') : 'color-3';
        ...
        console.log('Theme sync not available for this iframe');
    }
}
